...
module.exports = {
    name: 'ecchi',
    description: 'Show Random Konachan Images (Questionable)\n**Only allowed in NSFW flagged channels.**',
    ...
    execute(message, args) {
        if (args[0]) {
            if (args.includes('loli') || args.includes('shota')) {
                ...
                url = `https://konachan.com/post.json?limit=1&tags=order:random%20rating:questionable ${args.join('_')}`;
            }
        } else {
            url = `https://konachan.com/post.json?limit=1&tags=order:random%20rating:questionable`;
        }
