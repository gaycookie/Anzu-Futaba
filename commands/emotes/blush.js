...
module.exports = {
    name: 'blush',
    description: 'Kawaii Emotes: Blush Emote',
    usage: '<@mention>',
    guildOnly: true,
    aliases: [],
    execute(message, args) {
        ...
        message.guild.fetchMember(message.client.user).then(clientMember => {
            ...
            if (!message.mentions.users.size) {
                return weeb_embed('blush', `**${author} is blushing, cutie~**`, message);
            }
            return weeb_embed('blush', `**${author} is blushing because of ${usersWithCommaAnd}, cutie~**`, message);
