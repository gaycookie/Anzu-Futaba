...
module.exports = {
    name: 'wag',
    description: 'Kawaii Emotes: Wag Emote',
    usage: '<@mention>',
    guildOnly: true,
    aliases: [],
    execute(message, args) {
        ...
        message.guild.fetchMember(message.client.user).then(clientMember => {
            ...
            if (!message.mentions.users.size) {
                return weeb_embed('wag', `**${author} waggles their tail, so kawaii~**`, message);
            }
            return weeb_embed('wag', `**${author} waggles their tail for ${usersWithCommaAnd}, so kawaii~**`, message);
