...
module.exports = {
    name: 'sleepy',
    description: 'Kawaii Emotes: Sleepy Emote',
    usage: '<@mention>',
    guildOnly: true,
    aliases: [],
    execute(message, args) {
        ...
        message.guild.fetchMember(message.client.user).then(clientMember => {
            ...
            if (!message.mentions.users.size) {
                return weeb_embed('sleepy', `**${author} is sleepy, yawns~**`, message);
            }
            return weeb_embed('sleepy', `**${author} is sleepy because of ${usersWithCommaAnd}, yawns~**`, message);
