...
module.exports = {
    name: 'nom',
    description: 'Kawaii Emotes: Nom Emote',
    usage: '<@mention>',
    guildOnly: true,
    aliases: [],
    execute(message, args) {
        ...
        message.guild.fetchMember(message.client.user).then(clientMember => {
            ...
            if (!message.mentions.users.size) {
                return weeb_embed('nom', `**${author} noms some yummies, yaay~**`, message);
            }
            return weeb_embed('nom', `**${author} noms some yummies with ${usersWithCommaAnd}, yaay~**`, message);
